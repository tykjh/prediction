...
const InputSection = ({ onAddEntry, existingPeriods, nextPeriod, activeGameConfig }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!/^\d{9}$/.test(cleanPeriod)) {
            setError('Period must be exactly 9 digits.');
            return;
        }
        if (existingPeriods.includes(cleanPeriod)) {
            setError(`Period ${cleanPeriod} already exists.`);
            return;
        }
        ...
        if (cleanDate && !/^\d{7}$/.test(cleanDate)) {
            setError('Date must be exactly 7 digits.');
            return;
        }

        // 3. Numbers Validation
        if (numbers.some(n => n === '')) {
            setError(`Please fill in all ${numbers.length} number fields.`);
            return;
        }
        ...
        if (invalidMain !== undefined) {
            setError(`Main Number ${invalidMain} is out of range. Must be 1-${MAX}.`);
            return;
        }
        ...
        if (uniqueMain.size !== mainNums.length) {
            setError('Duplicate main numbers detected. Main numbers must be unique.');
            return;
        }
        ...
        if (HAS_SPECIAL && specialNum !== null) {
            ...
            if (specialNum < 1 || specialNum > SPECIAL_MAX) {
                setError(`Special Number ${specialNum} is out of range. Must be 1-${SPECIAL_MAX}.`);
                return;
            }
            ...
            if (!IS_SEPARATE) {
                if (mainNums.includes(specialNum)) {
                    setError(`Special Number ${specialNum} cannot duplicate a main number in this game.`);
                    return;
                }
            }
        }
    ...
    return (
        <div className="bg-slate-900/40 light:bg-gradient-to-br light:from-white light:to-slate-100/90 backdrop-blur-md rounded-2xl p-8 border border-white/5 light:border-slate-200/60 shadow-inner light:shadow-lg h-full flex flex-col justify-center relative overflow-hidden group">
            ...
            <h2 className="text-xl font-bold mb-6 text-white flex items-center gap-3 relative z-10">
                ...
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400 light:from-slate-700 light:to-slate-500">Add New Draw</span>
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-slate-400 light:text-slate-600 text-xs uppercase font-bold mb-1 ml-1">Period (9 Digits)</label>
                        <input
                            type="text"
                            value={period}
                    ...
                    <div>
                        <label className="block text-slate-400 light:text-slate-600 text-xs uppercase font-bold mb-1 ml-1">Date (7 Digits)</label>
                        <input
                            type="text"
                            value={date}
                ...
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label className="block text-slate-400 text-xs uppercase font-bold">Winning Numbers</label>
                        <span className="text-[10px] text-slate-500">
                            {(() => {
                                const SPECIAL_MAX = activeGameConfig?.settings?.specialNumber?.max || MAX;
                                if (IS_SEPARATE) return `1-${MAX} (Main) + 1-${SPECIAL_MAX} (Special)`;
                                return `1-${MAX} Only (All Unique)`;
                            })()}
                        </span>
                    </div>
