...
def parse_539():
    ...
    input_path = os.path.join(base_dir, 'raw_539.txt')
    output_path = os.path.join(base_dir, 'history_539.json')

    if not os.path.exists(input_path):
        print(f"Error: Input file not found at {input_path}")
        print("Please create 'raw_539.txt' with the lottery data.")
        return
    ...
    # We need to collect 5 numbers per period
    collected_numbers = []
    ...
    for line in lines:
        ...
        if line.startswith('第') and line.endswith('期'):
            ...
            if current_entry and 'period' in current_entry and collected_numbers:
                # We expect 5 numbers
                if len(collected_numbers) == 5:
                    # Sort all 5
                    main_nums = sorted([int(n) for n in collected_numbers])
                    current_entry['numbers'] = main_nums
                    history.append(current_entry)
                else:
                    print(f"Warning: Period {current_entry.get('period')} has {len(collected_numbers)} numbers (expected 5). Skipping.")
        ...
        if line.startswith('開獎日期:'):
            if state == 'WAITING_FOR_DATE' or state == 'WAITING_FOR_PERIOD': 
    ...
    if current_entry and 'period' in current_entry and collected_numbers:
        if len(collected_numbers) == 5:
            ...
            print(f"Warning: Last Period {current_entry.get('period')} has {len(collected_numbers)} numbers (expected 5). Skipping.")
